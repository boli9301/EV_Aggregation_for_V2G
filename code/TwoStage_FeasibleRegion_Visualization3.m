function [] = TwoStage_FeasibleRegion_Visualization3(P_input , P_compare1 , P_compare2)
    %% 绘图---compare1为精确--compare2为基集
    disp('--- 两阶段灵活性可视化 3对比---');
    ...
    if ~P_compare1_2d.isEmptySet
        ...
        if ~isempty(V_compare1) && size(V_compare1, 2) >= 2
            ...
            legend_str{end+1} = sprintf('输入2 (面积=%.0f)', area_compare1);
        end
    end
    ...
    if ~P_compare2_2d.isEmptySet
        ...
        if ~isempty(V_compare2) && size(V_compare2, 2) >= 2
            ...
            if area_compare2 > 1e-6
                legend_str{end+1} = sprintf('输入3灵活性 (面积=%.0f, %.1f%%)', area_compare2, (area_compare2/area_compare1)*100);
            else
                legend_str{end+1} = sprintf('基集出错');
            end
        end
    end
